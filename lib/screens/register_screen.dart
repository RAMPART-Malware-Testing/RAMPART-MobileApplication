...
class _RegisterScreenState extends State<RegisterScreen> with TickerProviderStateMixin {
  ...
  void _handleRegister() async {
    ...
    if (!_isRecaptchaVerified) {
      _showSnackBar('กรุณายืนยันตัวตนด้วย reCAPTCHA ก่อน', Colors.redAccent, icon: Icons.security_rounded);
      return;
    }
    ...
    if (res['success']) {
      ...
      Get.offAllNamed('/confirm-otp', arguments: {"type": "register"});
    } else {
        ...
        child: Column(
          children: [
            ...
            _buildTextField(
              ...
              label: 'Username',
              hint: 'rampart analysis',
              icon: Icons.supervised_user_circle_rounded,
              validator: (v) {
                if (v == null || v.isEmpty) return 'กรุณากรอก Username';
                if (v.length <= 3) return 'Username ต้องมีความยาวมากกว่า 3 ตัวอักษร';
                return null;
              },
            ),
  ...
  Widget _buildRecaptcha() {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          children: [
            Icon(_isRecaptchaVerified ? Icons.check_box : Icons.check_box_outline_blank, color: _isRecaptchaVerified ? Colors.green : _hintColor),
            const SizedBox(width: 12),
            Text(
